    ...
    void main(){
        ...
        vec3 color = texture2D(gcolor, texcoord).rgb;
            ...
            // Recreate our lighting model if it were only shading a single pixel
                ...
                // Apply exposure
                color /= max(getLuminance((lightCol * isEyeInWater * (1.0 - eyeBrightFact) * VOL_LIGHT_BRIGHTNESS + ambientLighting + nightVision + sqrt(torchBrightFact) * BLOCK_LIGHT_COL + cubed(eyeBrightFact) * (lightCol + skyCol)) * 0.36) * AUTO_EXPOSURE_MULT, MIN_EXPOSURE_DENOM);
            #else
                // Apply exposure
                color /= max(getLuminance((lightCol * isEyeInWater * (1.0 - SKY_LIGHT_AMOUNT) * VOL_LIGHT_BRIGHTNESS + ambientLighting + nightVision + sqrt(torchBrightFact) * BLOCK_LIGHT_COL + cubed(SKY_LIGHT_AMOUNT) * (lightCol + skyCol)) * 0.36) * AUTO_EXPOSURE_MULT, MIN_EXPOSURE_DENOM);
            #endif
        #else
            float accumulatedLumi = 1.0;
            color /= max(accumulatedLumi * AUTO_EXPOSURE_MULT, MIN_EXPOSURE_DENOM);
