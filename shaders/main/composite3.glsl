...
INOUT vec2 texcoord;
    ...
    void main(){
            ...
            vec3 eBloom = vec3(0);
                ...
                eBloom += texture2D(colortex2, scaledUv + vec2(0, pixelSize) * 2.4, 2.0 * BLOOM_LOD).rgb * 0.0625;
                eBloom += texture2D(colortex2, scaledUv + vec2(0, pixelSize) * 1.2, BLOOM_LOD).rgb * 0.25;
                eBloom += texture2D(colortex2, scaledUv, int(sqrt(BLOOM_LOD))).rgb * 0.375;
                eBloom += texture2D(colortex2, scaledUv - vec2(0, pixelSize) * 1.2, BLOOM_LOD).rgb * 0.25;
                eBloom += texture2D(colortex2, scaledUv - vec2(0, pixelSize) * 2.4, 2.0 * BLOOM_LOD).rgb * 0.0625;
            #elif BLOOM_QUALITY == 3
                eBloom += texture2D(colortex2, scaledUv + vec2(0, pixelSize) * 3.6, 3.0 * BLOOM_LOD).rgb * 0.015625;
                eBloom += texture2D(colortex2, scaledUv + vec2(0, pixelSize) * 2.4, 2.0 * BLOOM_LOD).rgb * 0.09375;
