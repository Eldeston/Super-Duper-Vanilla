...
vec3 getFogRender(vec3 eyePlayerPos, vec3 color, vec3 fogCol, float worldPosY, bool skyMask){
    ...
    float c = FOG_TOTAL_DENSITY_FALLOFF * (isEyeInWater * 2.56 + rainMult) * 1.512;
    float b = FOG_VERTICAL_DENSITY_FALLOFF * 1.512;
    float fogMult = min(1.0, FOG_OPACITY * MIST_GROUND_FOG_BRIGHTNESS * (rainMult + isEyeInWater * 0.256) * 1.256);
