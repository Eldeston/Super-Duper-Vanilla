...
float getSunMoonShape(vec3 pos){
    return smoothstep(0.0004, 0.0002, length(cubed(pos.xy)));
}
...
vec3 getSkyRender(vec3 playerPos, float skyMask, vec3 skyCol, vec3 lightCol){
    ...
    if(isEyeInWater >= 1){
        ...
        float newSkyData = max(star, sunMoon * 5.0) * skyMask * 2.0;
...
vec3 getSkyRender(vec3 playerPos, float skyMask, vec3 skyCol, vec3 lightCol){
    ...
    vec3 fogCol = skyCol * (0.5 * (1.0 - voidGradient) + voidGradient) * 0.75;
    vec3 final = saturate(mix(mix(fogCol, skyCol, skyFogGradient), lightCol + cubed(lightRange) * 0.25, lightRange));
    return mix(final, lightCol + newSkyData, newSkyData);
}