...
vec3 rayTraceScene(vec3 screenPos, vec3 viewPos, vec3 rayDir){
	...
	clipPosRayDir = normalize(clipPosRayDir - clipPos) * (2.0 / 32.0);

	// vec3 startPos = clipPos;
	for(int x = 0; x < 30; x++){
		clipPos += clipPosRayDir;
		vec3 newScreenPos = clipPos * 0.5 + 0.5;
		if(newScreenPos.x < 0.0 || newScreenPos.y < 0.0 || newScreenPos.x > 1.0 || newScreenPos.y > 1.0) return vec3(0);
		float depth = texture2D(depthtex0, newScreenPos.xy).x;
		// float delta = depth - newScreenPos.z;

		if(newScreenPos.z > depth) return vec3(binarySearch(clipPosRayDir, clipPos).xy * 0.5 + 0.5, depth != 1);
		// if(newScreenPos.z > depth && (newScreenPos.z - depth) < 0.001)
	}
	return vec3(0);
}