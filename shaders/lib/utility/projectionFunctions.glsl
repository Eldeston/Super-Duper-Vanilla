...
vec3 getScreenPos(in mat4 projection, in vec3 viewPos){
    ...
    vec3 clipCoord = vec3(projection[0].x, projection[1].y, 2) * viewPos;
    return 0.5 - vec3(clipCoord.xy / clipCoord.z, projection[3].z / clipCoord.z + projection[2].z * 0.5);
}
...
vec2 getScreenCoord(in mat4 projection, in vec3 viewPos){
    ...
    vec2 clipCoord = vec2(projection[0].x, projection[1].y) * viewPos.xy;
    return 0.5 - clipCoord.xy / (viewPos.z * 2.0);
}
