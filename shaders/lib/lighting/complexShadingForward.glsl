vec4 complexShadingGbuffers(matPBR material, positionVectors posVector, float dither){
	...
	vec3 totalDiffuse = (skyCol * material.light.y * material.light.y + ambientLighting + (material.light.x * material.light.x * material.light.x * material.light.x * (BLOCKLIGHT_I * 0.00392156863)) * vec3(BLOCKLIGHT_R, BLOCKLIGHT_G, BLOCKLIGHT_B)) * material.ambient;
